...
(lisp-implementation-version)  ; => "0.11.0"
```
...
- **Upstream `bordeaux-threads` and Quicklisp patches** — close the remaining `MP`/BT v2 API gaps (semaphores, `with-timeout`, and distinct lock types so `native-lock-p` / `native-recursive-lock-p` / `recursive-lock-p` can tell them apart) so the fork's `impl-clamiga.lisp` backends can merge upstream; same for the Quicklisp network/HTTP adaptations currently in `lib/quicklisp-compat.lisp`
