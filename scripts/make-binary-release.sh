...
# Workbench icons: three IconX project icons in the package root, each with
# its launcher script of the same name (icons/, generated by
# scripts/make-icons.py).  A double-click on CLAmiga / CLAmiga-FPU opens a
# console and starts that clamiga; Clamacs starts the editor.  The scripts
# pick bin/mos on MorphOS themselves.
...
cat >> "$STAGE/README-BINARY.txt" <<EOF
  ...
  CLAmiga, CLAmiga-FPU, Clamacs
                        ...
                        matching binary from bin/ (see Workbench below)
...
scripts start the bin/mos binaries.  The console's size and title are the
WINDOW tool type of the CLAmiga icons (Icons > Information); closing that
window ends clamiga.
