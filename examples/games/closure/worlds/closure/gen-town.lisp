...
;;; (gfx/house-0..2.iff) picked by a seeded PRNG — the same seed
;;; always writes the same town, so the map is checked in and this
;;; script only reruns to change the layout.
;;;
...
(defun add-house (x y side n)
  (carve-door x y side)
  (let ((style (rnd 3))
        (holder (aref *householders* (mod n (length *householders*)))))
    (add-special x y
                 (format nil "(location \"~A — the ~A's\" :house~%~
                              ~11T:image \"gfx/house-~D.iff\")"
                         (aref *house-styles* style) holder style))))
  ...
  (loop for by from 0 below (length *bands*)
        ...
        do (loop for bx from 0 below (length *bands*)
                 ...
                 do (solid-block x0 y0 x1 y1)
                    ;; one or two doors, on random sides at random spots
                    (dotimes (d (1+ (rnd 2)))
...
(with-open-file (out "worlds/closure/town.map"
  ...
  (format out ";; The town of Closure — the campaign's city, Skara Brae~%~
               ...
               ;; three facade pictures (gfx/house-0..2.iff); the shoppe and~%~
               ;; the tavern (trapdoor to cellar.map below) flank the south~%~
               ;; gate where the party starts.~%~%")
  ...
  (maphash (lambda (cell ops)
             ...
             (let ((loc (find-if (lambda (op)
               ...
               (when (and loc (eq (third loc) :house))
                 (incf houses)
                 (pushnew (getf (cdddr loc) :image) images
                          :test #'equal))))
