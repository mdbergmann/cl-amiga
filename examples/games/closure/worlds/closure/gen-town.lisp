...
;;; picked by a seeded PRNG, so the same seed always writes the same
;;; town, the map is checked in and this script only reruns to change
;;; the layout.
;;;
...
(with-open-file (out "worlds/closure/town.map"
  ...
  (format out ";; The town of Closure — the campaign's city, Skara Brae~%~
               ...
               ;; pieces to the matching pack variant.  The shoppe and the~%~
               ;; tavern (trapdoor to cellar.map below) flank the south gate~%~
