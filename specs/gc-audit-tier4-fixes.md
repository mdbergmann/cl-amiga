...
- [x] **AH5 (7b)** — hash_obj content-hashes wide strings (low-byte rotate-XOR, consistent with narrow cl_hash_string for EQUAL-equal content), bit-vectors (active length + bits), and EQUALP vectors (length + first element, mirroring the cons rule); keys_equal gained the missing bit-vector branch (CLHS EQUAL) and now EQ-keys general vectors under EQUAL (matching the conformant EQUAL builtin — the old content descent violated the hash contract); EQUALP char compare uses int, killing the (char)-cast false match (U+4E41 ≡ #\A). gc_key_addr_sensitive kept in sync (wide strings/bit-vectors content-stable under equal/equalp).
...
- [x] **Reader token caps (7b)** — read_atom_with_prefix (+ its prefix), read_radix_number, read_string, #: names, #* bits, and #\ names all signal CL_ERR_PARSE reader errors at their caps instead of silently splitting/truncating (a 300-digit integer used to read as a 255-digit number + a second number).
- [x] **R-srcloc (7c)** — dead keys cleared right after marking (a later object at the same offset false-matched → wrong file/line) and surviving keys forwarded while the fwd table is alive (gc_srcloc_invalidate_dead/gc_srcloc_forward in mem.c); srcloc_record uses ordered writes (invalidate key first) so unlocked MT readers never pair a new key with an old entry.
- [x] **ST7+ST8+ST9 (7c)** — file-position/file-length return bignums past CL_FIXNUM_MAX and file-position accepts a bignum position; the sweep finalizer closes dead never-CLOSEd file/socket handles with a loud stderr warning (via safe-region-free GC variants: platform_file_close is already clean, new platform_socket_close_gc skips the flush + its safe-region bracket — a safe-region enter on the collecting thread is forbidden); stream-layer roots registered BEFORE the interning in cl_builtins_stream_init.
- [x] **map family (7b)** — mapl/mapcon coerce function designators like their siblings; the silent 16-list/seq clamps in mapcar/mapc/mapcan/maplist/mapl/mapcon/map/map-into/every/some are loud CL_ERR_ARGS errors.
...
- [x] **repl + trace (7b)** — both REPL accumulators print a diagnostic and discard the form on overflow instead of evaluating a silently-truncated prefix; bi_trace_function/bi_untrace_function do the CL_SYM_TRACED flag + cl_trace_count RMW under cl_tables_wrlock (list cons stays outside the lock — STW-vs-rwlock).
- [x] **FS16 (7c)** — new cl_dynbind_c (thread.c) mirrors OP_DYNBIND for C builtins; bi_write, bi_pprint, bi_write_to_string, FORMAT's render_integer, and the four printer.c prin1/princ toggles all install THREAD-LOCAL binds popped by cl_dynbind_restore_to (dyn stack is GC-marked → the manual restore-class roots and cl_print_controls_save/restore are gone). Pre-fix a peer thread sampling *PRINT-BASE* during another thread's (write-to-string x :base 2) saw base 2 in ~98% of samples; post-fix zero (tests/test_mt_print_controls.sh).
